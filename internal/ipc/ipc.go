...
func WriteRequest(writer io.Writer, request Request) error {
	return write(writer, request, MaxRequestBytes)
}
func ReadRequest(reader io.Reader) (Request, error) {
	var request Request
	return request, read(reader, &request, MaxRequestBytes)
}
func WriteReply(writer io.Writer, reply Reply) error { return write(writer, reply, MaxReplyBytes) }
func ReadReply(reader io.Reader) (Reply, error) {
	var reply Reply
	return reply, read(reader, &reply, MaxReplyBytes)
}

func write(writer io.Writer, value any, maximum int) error {
	encoded, err := json.Marshal(value)
	if err != nil {
		return fmt.Errorf("encode IPC: %w", err)
	}
	if len(encoded) > maximum {
		return fmt.Errorf("IPC message exceeds %d bytes", maximum)
	}
	var length [4]byte
...
func read(reader io.Reader, destination any, maximum int) error {
	...
	if size == 0 || size > uint32(maximum) {
		return fmt.Errorf("invalid IPC message size %d", size)
	}
	...
	if err := json.Unmarshal(encoded, destination); err != nil {
		return fmt.Errorf("decode IPC: %w", err)
	}
	return nil
}
