...
| Candidate Qwen models | The three named Qwen weights were downloaded only to the maintainer's local Ollama store for the 2026-09-06 evaluation; they are not bundled in source or release archives. Their installed manifests display Apache-2.0 and are pinned in `EXECPLAN-LIVE-MODEL.md`. | A local evaluation and a model manifest are not redistribution authority. Retain applicable terms/notices and review the exact digest and redistribution rights before shipping any weight. |
